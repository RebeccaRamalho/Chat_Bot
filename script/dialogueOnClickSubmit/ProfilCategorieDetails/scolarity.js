...
document.querySelector("body").addEventListener("click", (event) => {
  ...
  let otherExperienceChoices = document.querySelector(
    "#otherExperienceChoices"
  );
  //à refaire plutôt donner cette valeur à genericScolarity: genericScolarity = genericScolarityButton.lenght-1 (et pareil genericScolariyAndYespour)
  let genericScolarityButton = document.querySelectorAll(".genericScolarity");
  let genericScolarity =
    document.querySelectorAll(".genericScolarity").length - 1;
  let genericScolariyAndYesButtonAll = document.querySelectorAll(
    ".genericscolariyAndYes"
  );
  let genericScolariyAndYes =
    document.querySelectorAll(".genericscolariyAndYes").length - 1;
  //
  let scolariyAndYes = document.querySelector(".scolariyAndYes");
  //
  ...
  function makeDotsMoves() {
    let compteurScolarityAndYesInt = Number(
      genericScolariyAndYesButtonAll[
        genericScolariyAndYes
  ...
  if (
    ...
    setTimeout(function () {
      ...
      experienceDetailsSub.innerHTML = `<p class="font">${experienceDetailsTexte2[0]}</p>`;
    }, 1500);
  ...
  //KEYBOARD AND CLICK RESPONSE HANDLER
  //2)if user use on of the category keyword (here relative to Scolarity)
  else if (
    event.target.matches("button[class='genericScolarity']>p") ||
    (event.target.matches("#submitMessage") &&
      (finalInputVal.includes("scolarité") ||
        finalInputVal.includes("scolarités") ||
        finalInputVal.includes("étude") ||
  ...
  ) {
    {
      document
        ...
        [genericScolarity].attributes[1].textContent.slice(9)
        ? (compteur2 = Number(
            document
              .querySelectorAll(".genericScolarity")
              [genericScolarity].attributes[1].textContent.slice(9)
    ...
    function makeDotsMove() {
      ...
      const firstGroupeOfDots = new Chargement(
        document.querySelector(`.categoriesChargingDots${compteur2}`),
        document.querySelector(`.categoriesDots${compteur2}`),
        document.querySelector(`.categoriesDotsOne${compteur2}`),
        document.querySelector(`.categoriesDotsTwo${compteur2}`),
        document.querySelector(`.categoriesDotsThree${compteur2}`)
      );
      const secondeGroupeOfDots = new Chargement(
        document.querySelector(`.categoriesChargingDots3${compteur2}`),
        document.querySelector(`.categoriesDots3${compteur2}`),
        document.querySelector(`.categoriesDotsOne3${compteur2}`),
        document.querySelector(`.categoriesDotsTwo3${compteur2}`),
        document.querySelector(`.categoriesDotsThree3${compteur2}`)
      );
      function firstDotsValues() {
        chargingDotsThree = firstGroupeOfDots.chargingDotsThree;
        dots = firstGroupeOfDots.dots;
    ...
    function chooseAnOtherExperience() {
      setTimeout(function () {
        //request to the DOM of the user response html element
        let otherExperiencesRequest = document.querySelector(
          `#experiences${compteur2}`
        );
        let yesQuestion = document.querySelector(`#yesButton${compteur2}`);
        let noQuestion = document.querySelector(`#noButton${compteur2}`);
